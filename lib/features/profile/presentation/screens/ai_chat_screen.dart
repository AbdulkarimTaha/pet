...
class _AIChatScreenState extends State<AIChatScreen> {
  ...
  Future<void> _sendMessage() async {
    ...
    setState(() {
      _messages.add({'text': message, 'sender': 'user', 'timestamp': DateTime.now()});
      _isLoading = true;
    });
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _messages.add({
            'text': responseData['reply'] ?? 'No response from AI',
            'sender': 'bot',
  ...
  void _handleError(String error) {
    setState(() {
      _messages.add({'text': error, 'sender': 'error', 'timestamp': DateTime.now()});
    });
    _scrollToBottom();
  }
